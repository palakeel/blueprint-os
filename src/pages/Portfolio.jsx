          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                ...
                {/* Cash row */}
                {activeAccount !== 'Crypto' && (
                  <tr className="border-t text-sm group" style={{ borderColor: 'var(--border)' }}>
                    <td className="px-4 py-3 font-bold" style={{ color: 'var(--text-secondary)', fontFamily: "'JetBrains Mono', monospace" }}>CASH</td>
                    <td className="px-4 py-3 text-right" style={{ color: 'var(--text-dim)' }}>—</td>
                    <td className="px-4 py-3 text-right" style={{ color: 'var(--text-dim)' }}>—</td>
                    {priceStatus === 'connected' && <td className="px-4 py-3" />}
                    <td className="px-4 py-3 text-right tabular-nums" style={{ color: 'var(--text-primary)', fontFamily: "'JetBrains Mono', monospace" }}>
                      <Private>{cashBalance > 0 ? `$${cashBalance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '—'}</Private>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button type="button" onClick={() => { setPanel('add'); setEditingPos(null) }}
                          className="p-1 rounded hover:opacity-70" style={{ color: 'var(--accent-cyan)' }}>
                          <Pencil size={11} />
                        </button>
                      </div>
